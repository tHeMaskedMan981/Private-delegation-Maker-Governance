...
export class Delegate extends Component {
    constructor(props){
        super(props);
        this.state = {account5Votes: null, show: false}
    }
    handleChange = (e) => {
    ...
    vote = async () => {
        ...
        let vA = proof.split('-')[0];
        let s_p = JSON.parse(atob(proof.split('-')[4]));
        console.log(s_p);
        secret_proof = s_p;
        // await DSChiefContractInstance.methods.delegateVote([this.state.account5], this.state.voteContractAddress, secret_proof["proof"]["a"],secret_proof["proof"]["b"],secret_proof["proof"]["c"],secret_proof["inputs"]).send({from:account});

        if(this.state.active == 5){
            await DSChiefContractInstance.methods.delegateVote([this.state.account5], this.state.voteContractAddress, secret_proof["proof"]["a"],secret_proof["proof"]["b"],secret_proof["proof"]["c"],secret_proof["inputs"]).send({from:account, gas: 4000000});
            await DSChiefContractInstance.methods.delegateVote([this.state.account5], this.state.voteContractAddress, secret_proof["proof"]["a"],secret_proof["proof"]["b"],secret_proof["proof"]["c"],secret_proof["inputs"]).send({from:account, gas: 4000000});
        }else{
            await DSChiefContractInstance.methods.delegateVote([this.state.account6], this.state.voteContractAddress, secret_proof["proof"]["a"],secret_proof["proof"]["b"],secret_proof["proof"]["c"],secret_proof["inputs"]).send({from:account, gas: 4000000});
            await DSChiefContractInstance.methods.delegateVote([this.state.account6], this.state.voteContractAddress, secret_proof["proof"]["a"],secret_proof["proof"]["b"],secret_proof["proof"]["c"],secret_proof["inputs"]).send({from:account, gas: 4000000});
        }
